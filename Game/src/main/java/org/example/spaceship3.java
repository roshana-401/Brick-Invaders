...
public class spaceship3 implements spaceships {
    private PApplet space;
    private PShape rocket;
    private int spaceshipX;
    private int spaceshipY = 725;

    public spaceship3(PApplet main, PShape rocket) {
        this.space = main;
        this.rocket = rocket;
    ...
    public boolean checkCrushed(int SpaceshipX, int SpaceshipY) {
        ...
        while (iterator.hasNext()) {
            ...
            if (meteo.getMeteoriteY() + 63 >= 690 && meteo.getMeteoriteY() + 63 <= 880) {
                int distance = (int) space.dist(getSpaceshipX() + 130, 855, meteo.getMeteoriteX() + 67, meteo.getMeteoriteY() + 63);
                if (distance <= 50 + 100 && Heart > 0) {
                    iterator.remove();
                    meteorites.remove(meteo);
                    collided = true;
                } else if (distance <= 50 + 100 && Heart == 0)
                    collided = true;
            }
            if (meteo.getMeteoriteY() + 63 > 880) {
                int distance = (int) space.dist(meteo.getMeteoriteX() + 67, meteo.getMeteoriteY() + 63, SpaceshipX + 130, 855);
                if (distance <= 50 + 100 && Heart > 0) {
                    iterator.remove();
                    meteorites.remove(meteo);
                    collided = true;
    ...
    public boolean checkCrushedOfGaint(int SpaceshipX, int SpaceshipY) {
        ...
        while (iterator.hasNext()) {
            ...
            if (meteo.getMeteoriteY() + 50 >= 690 && meteo.getMeteoriteY() + 50 <= 880) {
                int distance = (int) space.dist(getSpaceshipX() + 130, 855, meteo.getMeteoriteX() + 53, meteo.getMeteoriteY() + 50);
                if (distance <= 50 + 100 && Heart > 0) {
                    iterator.remove();
                    meteorites.remove(meteo);
                    collided = true;
            ...
            if (meteo.getMeteoriteY() + 50 > 880) {
                int distance = (int) space.dist(meteo.getMeteoriteX() + 53, meteo.getMeteoriteY() + 50, SpaceshipX + 130, 855);
                if (distance <= 50 + 100 && Heart > 0) {
                    iterator.remove();
                    meteorites.remove(meteo);
                    collided = true;
        ...
        while (iterator.hasNext()) {
            ...
            if (h.getY()+40 >= 700) {
                if (h.getY()+40 >= 695 && h.getY()+40 <= 880) {
                    int distance = (int) space.dist(getSpaceshipX() + 130, 855, h.getX()+40, h.getY()+40);
                    if (distance <= 80 + 60) {
                        iterator.remove();
                        Main.heart = replace;
                        return false;
                    ...
                    int distance = (int) space.dist(h.getX()+40, h.getY()+40, SpaceshipX + 130, 855);
                    if (distance <= 80 + 60) {
                        iterator.remove();
                        Main.heart = replace;
                        return false;
